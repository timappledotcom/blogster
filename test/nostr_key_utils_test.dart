...
import '../lib/utils/nostr_key_utils.dart';
...
void main() {
  group('NostrKeyUtils', () {
    ...
    test('should handle invalid nsec format gracefully', () {
      expect(() => NostrKeyUtils.nsecToHex('invalid'), throwsA(isA<ArgumentError>()));
      expect(() => NostrKeyUtils.nsecToHex('nsec1invalid'), throwsA(isA<ArgumentError>()));
    });

    test('should handle invalid hex format gracefully', () {
      expect(() => NostrKeyUtils.hexToNsec('invalid'), throwsA(isA<ArgumentError>()));
      expect(() => NostrKeyUtils.hexToNsec('zzzz'), throwsA(isA<ArgumentError>()));
    });
  });
}
