class BlogsterDocument {
  ...
  factory BlogsterDocument.fromJson(Map<String, dynamic> json) => BlogsterDocument(
    id: json['id'],
  ...
  static String generateFilename(String title) {
    ...
    if (filename.isEmpty) {
      final now = DateTime.now();
      filename = 'untitled_${now.year}${now.month.toString().padLeft(2, '0')}${now.day.toString().padLeft(2, '0')}_${now.hour.toString().padLeft(2, '0')}${now.minute.toString().padLeft(2, '0')}';
    }
